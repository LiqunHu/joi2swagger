...
let baseUrl = "http://${stageVariables.url}";
let relativeValidatorPath;
let validatorFile;
    ...
    if (currentValue.JoiSchema) {
      if (currentValue.JoiSchema.header) {
        ...
        for (let headerKey in swagger.properties) {
          ...
          requestMap[
            `integration.request.header.${headerKey}`
          ] = `method.request.header.${headerKey}`;
        }
      }
      if (currentValue.JoiSchema.body) {
        ...
        const modelName = `${currentValue.enname.replace(
          /\s/g,
          ""
        )}${currentValue.type.capitalize()}Body`;
        ...
        for (let queryKey in swagger.properties) {
          parameters.push({
            ...
            required: swagger.required
              ? swagger.required.includes(queryKey)
              : false,
      ...
      if (currentValue.JoiSchema.response) {
        ...
        for (let statusCode in swagger.properties) {
          const modelName = `${currentValue.enname.replace(
            /\s/g,
            ""
          )}${currentValue.type.capitalize()}${statusCode}Response`;
          json.components.schemas[modelName] =
            swagger.properties[statusCode].properties.body;

          const data = {
            description:
              swagger.properties[statusCode].properties.description.enum[0],
            schema: {
              $ref: `#/components/schemas/${modelName}`,
            },
          };
          ...
          if (swagger.properties[statusCode].properties.header) {
            data["headers"] =
              swagger.properties[statusCode].properties.header.properties;

            for (let headerName in swagger.properties[statusCode].properties
              .header.properties) {
              mapHeader[
                `integration.response.header.${headerName}`
              ] = `method.response.header.${headerName}`;
            }
          }
      ...
      if (
        currentValue.JoiSchema.deprecated &&
        currentValue.JoiSchema.deprecated === true
      ) {
        deprecated = true;
      }
    }
...
function getApiGatewayIntegration(
  currentValue,
  convertedPath,
  mapHeader,
  requestMap
...
function joi2swagger(argv) {
  if (argv.baseUrl) {
    baseUrl = argv.baseUrl;
  }
  relativeValidatorPath = argv.validator;
  validatorFile = path.resolve(relativeValidatorPath);
  ...
  if (argv.r) {
    glob(path.join(validatorFile, "**/*.validator.js"), function (er, files) {
      ...
      if (!fs.pathExistsSync(headerFile)) {
        return console.error(
          `Header file not found in ${headerFile}, please create header file first`
        );
      } else {
        try {
    ...
    if (!fs.pathExistsSync(validatorFile)) {
      console.error(
        `Validator file not found in ${validatorFile}, please create validator file first`
      );
      process.exit(1);
    } else {
      try {
